...
test("guest principal is denied persistence and administration but may read media", async () => {
  ...
  assert.equal(await guard.authorize(request("GET", "/api/jobs"), response()), true);
  for (const [method, path] of [["GET", "/api/files"], ["GET", "/api/playback/history"], ["POST", "/api/playback/events"], ["PATCH", "/api/cinema/watchlist"], ["GET", "/api/admin/backups"], ["GET", "/api/auth/accounts"]]) {
    const result = response();
    assert.equal(await guard.authorize(request(method, path), result), false, `${method} ${path}`);
    assert.equal(result.status, 403);
  }
  const deniedJobWrite = response();
  assert.equal(await guard.authorize(request("POST", "/api/jobs"), deniedJobWrite), false);
  assert.equal(deniedJobWrite.status, 403);
  db.close();
});
