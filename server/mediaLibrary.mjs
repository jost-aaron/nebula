...
export const scanMediaLibrary = async (storage, metadata, { mediaKind }, folder = storage.contentRoot, entries = []) => {
  for (const dirent of await readdir(folder, { withFileTypes: true })) {
    ...
    entries.push({
      ...
      ratingVotes: Number.isFinite(Number(mediaMetadata.ratingVotes)) ? Number(mediaMetadata.ratingVotes) : null,
      releaseYear: mediaMetadata.releaseYear,
      runtimeSeconds: Number.isFinite(Number(mediaMetadata.runtimeMinutes)) && Number(mediaMetadata.runtimeMinutes) > 0
        ? Number(mediaMetadata.runtimeMinutes) * 60
        : null,
      seriesRating: mediaMetadata.seriesRating || "",
      seriesRatingVotes: Number.isFinite(Number(mediaMetadata.seriesRatingVotes)) ? Number(mediaMetadata.seriesRatingVotes) : null,
      seriesRuntimeMinutes: Number.isFinite(Number(mediaMetadata.seriesRuntimeMinutes)) ? Number(mediaMetadata.seriesRuntimeMinutes) : null,
